<template>
    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li  v-bind:class="{'check':checkedIndex==index}" v-for="(item,index) in addressFilter" :key="index" @click="checkedIndex=index">
                <dl>
                  <dt>{{item.userName}}</dt>
  ...
  <modal v-bind:mdShow="modalConfirm" v-on:close="closeModal">
      <!-- vue3.0后版本用v-slot写法渲染template模块 -->
      ...
      <template v-slot:btnGroup>
        ...
        <!-- 这里的点击事件可以直接写@click="modalConfirm=flase" -->
      </template>
    </modal>
...
export default {
 ...
 //只要是在二次渲染前初始化就可以，这里的函数可以写成mouted，beforecreated都可以。
 created(){
   this.init();
 },
 methods:{
